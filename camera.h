...
color camera::ray_color(const Ray &r, const hittable &world, int depth)
{
    ...
    if (world.hit(r, 0.001, infinity, rec))
    {
        ...
        /*
        color surface_color = attenuation * ray_color(scattered, world, depth - 1);
        double fog_density = 0.03;
        double fog =exp(-rec.t * fog_density);

        color fog_color(0.7, 0.8, 1.0);

        return fog * surface_color + (1.0 - fog) * fog_color;

        color surface_color=0.5 * (rec.normal + color(1,1,1));
        double fog_density=0.08;
        double fog =exp(-rec.t * fog_density);
        color fog_color(0.7, 0.8, 1.0);
        return fog*surface_color+(1.0-fog)*fog_color;
        */
